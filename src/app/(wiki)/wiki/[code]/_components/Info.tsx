...
function Info({ userData, image, toggleActive, isToggleActive }: ProfileCardProps) {
  return (
    <section
      className={`flex flex-col rounded-xl border-none px-5 py-3 shadow-lg shadow-gray-300 transition-all duration-300 ease-in-out md:px-5 md:py-6 xl:h-[671px] xl:max-w-[320px] xl:p-[30px] ${isToggleActive ? "h-[260px] md:h-[290px]" : "h-[120px] md:h-[135px]"}`}
    >
      <article className="flex gap-5 md:gap-8 xl:flex-col xl:gap-[30px]">
        <div className="relative h-[62px] w-[62px] overflow-hidden rounded-full md:h-[71px] md:w-[71px] xl:m-[30px] xl:h-[200px] xl:w-[200px]">
          <Image layout="fill" className="object-contain" src={image || UserDafalutImage} alt="프로필이미지" />
        </div>

        <div
          className={`flex flex-col gap-2 truncate transition-all duration-300 ease-in-out xl:h-[304px] xl:gap-4 ${isToggleActive ? "h-[250px] md:h-[234px]" : "h-[70px] md:h-[80px]"}`}
        >
          {Object.entries(userData).map(([key, value]) => (
            <div key={key} className="flex gap-5">
              <div className="text-12 md:text-14 text-gray-400">{labels[key as keyof InfoType]}</div>
      ...
      <button className="mx-auto xl:hidden" type="button" onClick={toggleActive}>
        <ArrowdownImage
          ...
          alt={`${isToggleActive ? "프로필 상세 닫기" : "프로필 상세 보기"}`}
        />
      </button>
    </section>
  );
